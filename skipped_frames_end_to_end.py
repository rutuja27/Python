...
def matching_frames(arr1, arr2):
    if(len(arr1) != len(arr2)):
        print('Skipped frames do not match')
...
def match_scores(scr_ind_front, scr_ind_side, scr, scr_side_gt, scr_front_gt):
    ...
    if(len(scr_ind_front) != 0):
        for ind in scr_ind_front:
            if( scr_side_gt[ind-1] == scr[ind-1]):
                ...
                print('Scores mismatch index', ind,  scr[ind-1], scr_side_gt[ind-1], scr_front_gt[ind-1])
    ...
    if(len(scr_ind_side) != 0):
        for ind in scr_ind_side:
            if(scr_front_gt[ind-1] == scr[ind-1]):
                ...
                print('Scores mismatch', ind,
                      scr[ind-1], scr_front_gt[ind-1], scr_side_gt[ind-1])
...
def main():
    ...
    for i in range(0,no_of_trials):
        ...
        ut.readcsvFile_int(nidaq_thres_imagegrab_cam0_file, nidaq_thres_imagegrab_cam0[i], 1)
        ut.readcsvFile_int(nidaq_thres_imagegrab_cam1_file, nidaq_thres_imagegrab_cam1[i], 1)
        ...
        imagegrab_side_skips_gt.append(np.argwhere(skips_side_gt[i]==1000).flatten())
        imagegrab_front_skips_gt.append(np.argwhere(skips_front_gt[i]==1000).flatten())
        ...
        if (len(imagegrab_front_skips[i]) != 0 and len(imagegrab_front_skips_gt[i]) != 0):
            ...
            if((len(front_skips[i]) - len(only_imagegrab_front[i])) == len(skips_process_front[i])):
                print('Frames skipped in front at imagegrab macth ones in process Scores')


        # match the score of indexes where either side/front
        match_scores(front_skips[i], side_skips[i], scores_jaaba[i], scores_side_gt[i], scores_front_gt[i])
